...
const ShopByBrands = async() => {
  ...
  return (
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-5"> 
        {brands?.map((brand) => (
            <Link href={`/brand/${brand?.slug?.current}`}
                className="bg-white w-34 h-24 flex item-center justify-center rounded-md overflow-hidden hover:shadow-lg"
                >
                {brand?.image && (
                    <Image
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-16 p-2 shadow-sm hover:shadow-(--color-shop_light_purple)e/20 py-5">
        {extraData?.map((item, index) => (
          <div
            key={index}
            className="flex items-center gap-3 group text-lightColor hover:text-(--color-shop_light_purple)"
          >
